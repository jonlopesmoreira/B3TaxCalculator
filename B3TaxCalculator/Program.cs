...
internal class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        var continuarButton = new TaskDialogButton("Continuar");
        var sairButton = new TaskDialogButton("Sair");

        var instructionPage = new TaskDialogPage
        {
            Caption = "Selecionar notas",
            Heading = "Selecione as notas de corretagem",
            Text = "Na próxima janela, escolha os arquivos PDF das notas de corretagem.",
            Icon = TaskDialogIcon.Information,
            AllowCancel = true,
            DefaultButton = continuarButton
        };

        instructionPage.Buttons.Add(continuarButton);
        instructionPage.Buttons.Add(sairButton);

        var instructionResult = TaskDialog.ShowDialog(instructionPage);
        if (instructionResult != continuarButton)
        {
            Environment.Exit(0);
        }
        ...
        if (files.Length == 0)
        {
            Console.WriteLine("Nenhum arquivo PDF selecionado.");
            return;
        }
